...
# ---------- Голосовые сообщения (SaluteSpeech, тот же Sber Studio) ----------
SALUTE_SPEECH_AUTH_KEY = os.environ.get("SALUTE_SPEECH_AUTH_KEY")
SALUTE_SPEECH_SCOPE = os.environ.get("SALUTE_SPEECH_SCOPE", "SALUTE_SPEECH_PERS")
...
_salute_speech_token_cache = {"token": None, "expires_at": 0}


def get_salute_speech_token():
    now = time.time()
    if _salute_speech_token_cache["token"] and _salute_speech_token_cache["expires_at"] - 30 > now:
        return _salute_speech_token_cache["token"]
    if not SALUTE_SPEECH_AUTH_KEY:
        return None
    try:
        resp = requests.post(
            GIGACHAT_OAUTH_URL,  # тот же общий шлюз авторизации Sber, что и у GigaChat
            headers={
                "Content-Type": "application/x-www-form-urlencoded",
                "Accept": "application/json",
                "RqUID": str(uuid.uuid4()),
            ...
            data={"scope": SALUTE_SPEECH_SCOPE},
            verify=False,
            timeout=15,
        )
        resp.raise_for_status()
        ...
        if expires_at > 10**12:
            expires_at = expires_at / 1000
        _salute_speech_token_cache["token"] = token
        _salute_speech_token_cache["expires_at"] = expires_at
        return token
    except Exception as e:
        logger.error(f"SaluteSpeech auth error: {e}")
        return None


def recognize_speech_sync(audio_bytes: bytes):
    """Синхронное распознавание голосового сообщения (OGG/Opus от Telegram). Возвращает текст или None."""
    ...
    try:
        ...
        logger.error(f"SaluteSpeech recognize error: {e}")
        return None
...
async def classify_free_text(text: str):
    """Просит AI понять тип сообщения (трата/доход/платёж по долгу/вопрос) и категорию.
    ...
    prompt = (
        ...
        '{"type": "expense" | "income" | "debt_payment" | "question", "label": "строка или null", "amount": число или null}\n\n'
        f"Категории расходов на выбор для label, если type=expense: {categories}.\n"
        "ВАЖНО: если конкретный предмет или товар не назван прямо как категория, определи категорию "
        "по его обычному назначению, а не бери первое попавшееся слово. Примеры:\n"
        "- «расчёска 300» → label=Красота (это предмет ухода/красоты)\n"
        "- «сигареты 250» или «вейп 500» → label=Вредные привычки\n"
        "- «такси 400», «метро 60», «бензин 2000» → label=Транспорт\n"
        "- «кофе 250», «обед 400», «хлеб 80» → label=Продукты\n"
        "- «кино 600», «игра 1200» → label=Развлечения\n"
        "- «крем 900», «маникюр 2000» → label=Красота\n"
        "- «футболка 1500» → label=Одежда\n"
        "- «таблетки 300», «врач 2000» → label=Здоровье\n"
        "Если ни одна категория явно не подходит — используй Другое, но старайся выбрать конкретную категорию из списка.\n\n"
        "Если type=income, label — источник поступления в свободной форме (Зарплата, Кешбек, Подарок и т.п.).\n\n"
        "Если сообщение явно про оплату/погашение долга, кредита, кредитки — type=debt_payment, "
        'label=null, amount — сумма платежа (например: «закрыла кредитку на 20000» → {"type":"debt_payment","label":null,"amount":20000}).\n\n'
        "Если сообщение не похоже ни на что из перечисленного (вопрос, просьба совета, инструкция) — "
...
def pay_debt(user_id: int, amount: float, name: str = None):
    ...
    row = None
    if name:
        # Сначала точное совпадение без учёта регистра, затем частичное (по подстроке)
        row = conn.execute(
            "SELECT id, remaining_amount FROM debts WHERE user_id = ? AND LOWER(name) = ?",
            (user_id, name.strip().lower()),
        ).fetchone()
        if row is None:
            row = conn.execute(
                "SELECT id, remaining_amount FROM debts WHERE user_id = ? AND LOWER(name) LIKE ?",
                (user_id, f"%{name.strip().lower()}%"),
            ).fetchone()
    if row is None:
        # Название не задано или не найдено — берём единственный/первый активный долг
        row = conn.execute(
...
async def debt_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "Твои долги:\n\n" + "\n".join(lines) + "\n\nЗаписать платёж — просто напиши сумму (например: 20000)"
...
async def paydebt_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 1:
        context.user_data["mode"] = "debt"
        await update.message.reply_text(
            "Просто напиши сумму платежа (например: 20000), можно с названием долга."
        )
        return
    amount = parse_amount(args[0])
    if amount is None:
    ...
    await reply_debt_paid(update, user_id, amount, name)


async def reply_debt_paid(update: Update, user_id: int, amount: float, name: str = None):
    remaining = pay_debt(user_id, amount, name)
    if remaining is None:
        await update.message.reply_text("Такой долг не найден. Проверь /debt.", reply_markup=MAIN_KEYBOARD)
        return
...
_LEADING_AMOUNT_PATTERN = re.compile(r"^([0-9\s+\-*/,.]+)\s*(.*)$")


async def process_incoming_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    user_id = update.effective_user.id
    ensure_user(user_id)
    text = text.strip()
    ...
    if mode == "debt":
        ...
        if match:
            amount = _safe_eval_amount(match.group(1))
            if amount is not None:
                context.user_data["mode"] = None
                debt_name = match.group(2).strip() or None
                await reply_debt_paid(update, user_id, amount, debt_name)
                return
        # Не похоже на сумму — сбрасываем режим и обрабатываем сообщение как обычно ниже
        context.user_data["mode"] = None
        mode = None

    # Никакой активной кнопки/режима нет — пробуем понять сообщение через AI
    # (свободный ввод трат/доходов, платежей по долгу и вопросы про финансы).
    if mode is None and GIGACHAT_AUTH_KEY:
        ...
        if parsed:
            ...
            if msg_type == "income" and amount:
                ...
                return
            if msg_type == "debt_payment" and amount:
                await update.message.reply_text("🤖 Похоже, это платёж по долгу. Записываю.")
                await reply_debt_paid(update, user_id, float(amount), None)
                return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await process_incoming_text(update, context, update.message.text)
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not SALUTE_SPEECH_AUTH_KEY:
        await update.message.reply_text(
            "Голосовые сообщения пока не подключены — нужен ключ SALUTE_SPEECH_AUTH_KEY. "
            "Смотри README, раздел про голосовые.",
            reply_markup=MAIN_KEYBOARD,
        )
        return
    ...
    try:
        ...
        await update.message.reply_text("Не получилось скачать голосовое сообщение.", reply_markup=MAIN_KEYBOARD)
        return

    text = await asyncio.to_thread(recognize_speech_sync, audio_bytes)
    if not text:
        await update.message.reply_text(
            "Не удалось распознать речь. Попробуй ещё раз или напиши текстом.", reply_markup=MAIN_KEYBOARD
        )
        return

    await update.message.reply_text(f"🎤 Распознал: «{text}»")
    await process_incoming_text(update, context, text)
